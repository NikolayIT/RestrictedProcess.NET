...
- **`Process/SandboxDesktop.cs`** — a throwaway `rp_<guid>` desktop with an SDDL security descriptor granting Everyone + RESTRICTED + ALL APPLICATION PACKAGES and a Low mandatory label (so both the restricted Low-IL token and a network-blocking AppContainer process can use it).
- **Network blocking** (`RestrictedProcessOptions.BlockNetworkAccess`, opt-in) — runs the child in an **AppContainer** with no capabilities, so the Windows Firewall denies its sockets. `RestrictedProcess` calls `CreateAppContainerProfile` (a derived SID alone is not enough — the OS needs a registered profile to launch into), grants "ALL APPLICATION PACKAGES" (S-1-15-2-1) read/execute on the executable via `icacls`, and adds the `PROC_THREAD_ATTRIBUTE_SECURITY_CAPABILITIES` attribute (`SecurityCapabilities` struct, no capabilities). The profile is deleted and the SID freed on dispose.
...
- AppContainer network blocking (`BlockNetworkAccess`) has two non-obvious requirements that are handled in code but easy to reintroduce: the environment block must carry the profile variables (`USERPROFILE`/`APPDATA`/`LOCALAPPDATA`/…) or `CreateProcessAsUser` fails with `ERROR_ENVVAR_NOT_FOUND`, and the desktop DACL must grant `ALL APPLICATION PACKAGES` or the child crashes on startup. It also needs the Windows Firewall / Base Filtering Engine service running to actually enforce the block. The AppContainer test uses a native networking check and `Assert.SkipUnless` so it is skipped (not failed) on a host with no egress.
